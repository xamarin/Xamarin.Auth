...
namespace Xamarin.Auth
{
...
#endif
    {
        ...
        /// <summary>
        /// Access or Refresh Token Revocation URL.
        /// </summary>
        /// <value>The URL used to revoke access or refresh tokens.</value>
        ...
        /// </summary>
        /// <returns>Revoke token async.</returns>
        /// <param name="revoke_token_url">Revoke token URL.</param>
        /// <param name="token">Token.</param>
        /// <param name="token_type_hint">Token type hint.</param>
        /// <see cref="https://tools.ietf.org/html/rfc7009"/> 
        public async Task RevokeTokenAsync(Uri revoke_token_url, string token, string token_type_hint)
        {
            ...
            {
                parameters.Add("token_type_hint", token_type_hint);
            }

            Uri uri = await this.GetInitialUrlAsync(parameters);
            HttpClient client = new HttpClient();
            HttpResponseMessage response = await client.GetAsync(this.RevokeTokenUrl);
            string text = await response.Content.ReadAsStringAsync().ConfigureAwait(false);
        ...
        /// <param name="token_type_hint">Token type hint.</param>
        /// <see cref="https://tools.ietf.org/html/rfc7009"/> 
        public async Task RevokeTokenAsync(string token, string token_type_hint)
        {
            await RevokeTokenAsync(this.RevokeTokenUrl, token, token_type_hint);
